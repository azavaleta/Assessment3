...
================*/
var humans = 
  users.filter(function(element, index, array){
  ...
  users.map(function(element, index, array){
     return parseInt(`${element.age}`);
      ...
      const reducer = (accumulator, currentValue) => accumulator + currentValue;
        var totalAge = (totalAge.reduce(reducer));
...
/*==============
    ...
    .filter() iterates through an array and adds the element to the
    new array if it returns true. And .find() only returns the first
    one found that matches the parameters given to it.
